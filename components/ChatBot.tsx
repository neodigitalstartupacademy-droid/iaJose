...
import { Send, FileText, Paperclip, Download, Zap, Mic, MicOff, Volume2, Sparkles, X, Play, CircleStop, Waves, CheckCircle, Trash2, ShieldAlert, RefreshCcw, Stethoscope, Heart, Target, ScrollText, Activity, BrainCircuit, Globe, MapPin, ExternalLink, Brain, Loader2, Rocket } from 'lucide-react';
...
const ChatBot: React.FC<ChatBotProps> = ({ distData, isOwner, initialIntent }) => {
  ...
  useEffect(() => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => console.warn("Geolocation permission denied. Google Maps tool will be less precise.")
      );
    }
  }, []);
          ...
          onmessage: async (msg: LiveServerMessage) => {
            ...
            if (msg.serverContent?.turnComplete) {
              setMessages(prev => [...prev, 
                ...
                { role: 'model', text: partialTranscript.model || "[Réponse Vocale]", timestamp: Date.now(), isLive: true }
              ]);
              setPartialTranscript({ user: '', model: '' });
            }
          }
  ...
  const handlePlayVoice = async (text: string, index: number) => {
    ...
    try {
      const base64Audio = await textToSpeech(text, activeVoiceName);
      if (base64Audio) {
  ...
  const handleSend = async (customText?: string) => {
    ...
    try {
      ...
      if (attachment) {
        ...
        const isMapQuery = /où|lieu|adresse|proche|autour|restaurant|magasin|trouver|neolife point|situé/i.test(textToSend);
        ...
        for await (const chunk of result) {
          ...
          if (metadata?.groundingChunks) {
            metadata.groundingChunks.forEach((c: any) => {
              if (c.web && !groundingSources.find(s => s.uri === c.web.uri)) {
                groundingSources.push({ title: c.web.title, uri: c.web.uri, type: 'web' });
              }
              if (c.maps && !groundingSources.find(s => s.uri === c.maps.uri)) {
                groundingSources.push({ title: c.maps.title, uri: c.maps.uri, type: 'maps' });
              }
            });
          }
          setMessages(prev => {
      ...
      setMessages(prev => {
        const next = [...prev];
        if (next[modelIdx]) next[modelIdx].text = "Anomalie souveraine détectée. Coach JOSÉ recalibre le flux. Veuillez reformuler.";
        return next;
      });
    ...
    <div className="flex flex-col h-full bg-white dark:bg-ndsa-dark relative overflow-hidden font-sans transition-colors duration-300">
           ...
           <div className="flex flex-col items-center gap-10 relative z-10 w-full max-w-2xl text-center">
              ...
              <div className="space-y-6 text-white w-full">
                ...
                <div className="min-h-[120px] p-8 bg-white/5 border border-white/10 rounded-[3rem] backdrop-blur-md shadow-2xl">
                  <p className="text-2xl font-black italic leading-tight text-slate-100">{partialTranscript.model || "Écoute stratégique..."}</p>
                </div>
              </div>
      ...
      <div className="sticky top-0 z-40 bg-white/90 dark:bg-slate-900/90 backdrop-blur-lg border-b border-slate-100 dark:border-slate-800 px-6 py-4 flex items-center justify-between">
         <div className="flex items-center gap-4">
           ...
           <span className="text-[11px] font-black text-slate-900 dark:text-white uppercase tracking-[0.3em]">COACH JOSÉ SOUVERAIN v4.5</span>
         </div>
         <div className="flex gap-4">
            <button onClick={() => setUseThinking(!useThinking)} className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${useThinking ? 'bg-blue-600 text-white shadow-lg' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700'}`}>
               <Brain size={14} className={useThinking ? 'animate-pulse' : ''} /> RÉFLEXION {useThinking ? 'ON' : 'OFF'}
            </button>
              ...
              <div className={`p-8 md:p-10 rounded-[3rem] text-[18px] leading-relaxed shadow-sm transition-all duration-300 ${msg.role === 'user' ? 'bg-slate-900 text-white border border-slate-800 shadow-xl' : 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white border border-slate-100 dark:border-slate-800 shadow-2xl'}`}>
                ...
                {msg.role === 'model' && msg.text && !msg.isLive && (
                  <div className="mt-8 pt-6 border-t border-slate-50 dark:border-slate-800 flex gap-6">
                    ...
                    >
                      {isTtsLoading === idx ? <Loader2 className="animate-spin" size={16} /> : <Play size={16} fill="currentColor" />}
                      VOIX JOSÉ
                    </button>
          ...
          <div className="flex-1 w-full relative">
            <div className={`relative flex items-end gap-4 bg-black border-2 ${useThinking ? 'border-blue-500 ring-8 ring-blue-500/5 shadow-[0_0_40px_rgba(37,99,235,0.1)]' : 'border-slate-800'} rounded-[3rem] p-5 transition-all duration-500 shadow-2xl focus-within:border-blue-600`}>
              <button onClick={() => setShowVoicePicker(true)} className="p-3 text-slate-500 hover:text-blue-400 transition-all shrink-0 flex flex-col items-center gap-1 group">
                ...
                <span className="text-[7px] font-black uppercase tracking-tighter opacity-40 group-hover:opacity-100">{activeVoiceName}</span>
              </button>
              ...
              <textarea
                ...
                placeholder={useThinking ? "Analyse complexe pour JOSÉ PRO..." : "Commandez l'intelligence JOSÉ..."}
                rows={1}
            ...
            {attachment && (
              <div className="absolute -top-14 left-10 bg-blue-600 text-white text-[10px] px-6 py-2 rounded-full font-black flex items-center gap-3 animate-in slide-in-from-bottom-2 shadow-xl border border-white/20">
      ...
      {showResetConfirm && (
        <div className="absolute inset-0 z-[100] bg-slate-950/80 backdrop-blur-xl flex items-center justify-center p-10 animate-in fade-in">
           <div className="bg-white dark:bg-slate-900 rounded-[4rem] p-16 max-w-xl w-full text-center border-4 border-slate-50 dark:border-slate-800 shadow-[0_60px_120px_rgba(0,0,0,0.5)]">
              ...
              <h3 className="text-4xl font-black text-slate-900 dark:text-white uppercase tracking-tighter mb-6">Effacer la Mémoire ?</h3>
              <p className="text-lg text-slate-500 dark:text-slate-400 font-medium mb-12">Cette action détruira définitivement tout l'historique de cette session souveraine.</p>
              <div className="grid grid-cols-2 gap-6">
                 <button onClick={() => setShowResetConfirm(false)} className="py-7 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-3xl font-black uppercase text-xs tracking-widest hover:bg-slate-200 transition-all">ANNULER</button>
                 <button onClick={() => { setMessages([{ role: 'model', text: "Mémoire réinitialisée. Nouvelle session active.", timestamp: Date.now() }]); setShowResetConfirm(false); }} className="py-7 bg-red-600 text-white rounded-3xl font-black uppercase text-xs tracking-widest shadow-2xl hover:bg-red-700 transition-all">CONFIRMER</button>
              </div>
           </div>
        </div>
      )}
      ...
      {showVoicePicker && (
        <div className="absolute bottom-[180px] left-10 right-10 md:left-[25%] md:right-[25%] bg-slate-950 border border-white/5 rounded-[4rem] p-12 shadow-[0_60px_160px_rgba(0,0,0,0.8)] z-[70] animate-in slide-in-from-bottom-10">
          <div className="flex items-center justify-between mb-10 border-b border-white/5 pb-8 text-white">
            <h4 className="text-[12px] font-black uppercase tracking-[0.5em]">Signature Vocale JOSÉ</h4>
          ...
          <div className="grid grid-cols-2 gap-6">
            {VOICES.map((v) => (
              <button
                ...
                className={`flex flex-col text-left p-8 rounded-[3rem] border-2 transition-all duration-300 ${selectedVoice === v.id ? 'bg-blue-600 border-blue-400 text-white shadow-2xl scale-105' : 'bg-white/5 border-transparent text-slate-400 hover:bg-white/10 hover:border-white/10'}`}
              >
